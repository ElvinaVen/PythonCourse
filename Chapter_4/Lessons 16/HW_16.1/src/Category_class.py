...
class Category:
    ...
    def add_product(self, product_object):
        ...
        :return:
        """
        # if isinstance(product_object, self.__class__):
        if isinstance(product_object, Product):
            self.__products.append(product_object)
            if product_object.product_quantity == 0:
                raise ValueError('Товар с нулевым количеством не может быть добавлен')
            return self.__products
        else:
            raise TypeError("Нельзя складывать любой другой объект.")
    ...
    def average_price(self):
        ...
        try:
            result = 0
            for product in self.__products:
                result += product.price * product.product_quantity
            return f'Средний чек {result / len(self.__products)}'
